...
namespace RaspberryPiDotNet
{
	...
	public class GPIOMem : GPIO
	{
		...
		static extern void bcm2835_spi_chipSelect(byte cs);

		[DllImport("libbcm2835.so", EntryPoint = "bcm2835_spi_setClockDivider")]
		static extern void bcm2835_spi_setClockDivider(ushort cs);

		[DllImport("libbcm2835.so", EntryPoint = "bcm2835_spi_writenb")]
		static extern void bcm2835_spi_writenb(byte[] buf, uint len);
		#endregion

		public enum ChipSelect : byte {
			Cs0 = 0,
			Cs1 = 1,
			Cs2 = 2,
			CsNone = 3
		...
		{
			bcm2835_spi_writenb (bytes, (uint)length);
		}

		public static void EndSpi ()
		{
			bcm2835_spi_end ();
		}
	}
